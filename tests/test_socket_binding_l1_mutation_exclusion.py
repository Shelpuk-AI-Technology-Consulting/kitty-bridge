...
`.system_design/TEST_SUITE.md` §8.2 enumerates fifteen modules that bind real
...
def test_registry_has_exactly_fifteen_entries() -> None:
    """The count is fixed by §8.2's enumeration of fifteen modules."""
    assert len(SOCKET_BINDING_L1_MODULES) == 15, (
        f"registry has {len(SOCKET_BINDING_L1_MODULES)} entries; §8.2 enumerates fifteen"
    )
