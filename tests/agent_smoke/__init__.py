...
network beyond loopback. Three tests ship with the package:
...
(the §8 missing-binary rule, KBR-132 shape, on both the override branch
and the default-lookup branch), and :class:`TestTimeoutKill` (the
safety-critical branch that reaps a hung child). The settings-precedence
claim (T-I6) and the live nightly scenarios (T-K11, T-I14) belong
