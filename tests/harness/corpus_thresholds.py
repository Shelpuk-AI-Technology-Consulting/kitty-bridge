...
_ENTRY_METADATA: Final[dict[str, tuple[str, str]]] = {
    ...
    "compaction_budget_under": (
        ...
        "max_messages_chars=None, server.py:7012). M4 complement (no oversized "
        ...
        "(server.py:6996), not the Anthropic-Messages shape the fixture commits — the "
    ...
    "compaction_budget_over": (
        ...
        "messages length the bridge measures with `_safe_size` (server.py:6996), not "
...
def build_compaction_budget_under() -> tuple[CapturedRequest, frozenset[Trigger], frozenset[Trigger]]:
    ...
    **CC-converted** messages (``_safe_size``, ``server.py:6996``); this builder
    ...
    ``server.py:7012``). On profiles whose derived ``messages_budget`` is
...
def _padded_messages(
    *,
    ...
    The bridge measures ``len(json.dumps(messages, ensure_ascii=False))`` on the
    CC-converted messages (``_safe_size`` at ``server.py:6996``), not the
