...
| **Project CLAUDE.md review-resolution discipline** | The repo-root `CLAUDE.md` is the instruction every Claude Code session acts on, yet nothing imports it, so its presence, tracking, ignore-status, and the four mandated review-resolution behaviours would otherwise drift silently. Landed with **KBR-286** as `tests/test_project_claude_md_review_discipline.py`. Presence is `Path.exists` + `git ls-files --error-unmatch`; ignore-status is `git check-ignore` with exit code 1 as the only acceptable state (it evaluates every ignore pattern form, not just a literal line match); the four behaviours are pinned by stable anchors inside the review-resolution section only, whitespace-normalised so a re-wrap cannot break them; the file is read with `encoding="utf-8"` (KBR-266 cp1252 trap). Self-guarded per the §6.2 rule: each clause is independently tested, and a dev-time mutation check confirmed each anchor's clause is detected by removal. |
