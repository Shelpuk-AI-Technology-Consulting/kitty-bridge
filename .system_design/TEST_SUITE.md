...
Thirty request-path rows — the original fourteen (M1–M11, M15, M16 and M17), two
...
drop, G31), KBR-271's M9a and M9b (the M9 fallback converter's cache-breakpoint drops,
G43's register half), KBR-55's M27 (the translated-route `top_k` drop), M28 (the empty
`stop_sequences` omit), and M29 (the string-form `stop` rewrite), and KBR-309's M30
(the translated-route `context_management` drop, beta-gated) and M31 (the multi-text-part
user-turn join collapse) — one response-path row (M12), and the routing row **M14** (§3.3.5),
which is listed here because the destination is a mutation surface the body cannot show.
Thirty-three rows in all. The former substitution row M13 is **withdrawn** — KBR-5 replaced it
with a downstream error, so it mutates nothing — leaving **thirty-two live** bridge-level rows.
...
| M3 | Truncate a tool result over 50,000 chars (`_TOOL_RESULT_TRUNCATION_LIMIT`) | `_truncate_oversized_tool_results` / `_truncate_oversized_responses_outputs` | A single tool result exceeds the limit | A single oversized result can exceed the model's window on its own. The truncation sites measure both string content (`len`) and list-form content (serialised JSON length) via `_tool_result_content_size` — KBR-223 closed the list-form gap KBR-169 recorded, so an oversized structured `tool_result` or list-form Responses `output` truncates here too, not on a later line. On `openai_subscription`'s Responses path the shipped body is built from the raw inbound body, so the mutation is performed there by `_truncate_oversized_responses_outputs` on the Responses-shaped `input` itself — same limit, same notice text; the CC-shape site truncates a copy that never ships on that route (KBR-169). |
| M4 | Truncate a tool result over the same limit, again, inside compaction | `_compact_messages` step 1 | Compaction ran **and** a `role: "tool"` message's content exceeds the limit | Second pass, CC-shape only. Distinct from M3: M3 is unconditional pre-processing, M4 fires only once compaction is already engaged; list-form `role: "tool"` content is measured by the same extractor (KBR-223). |
...
| M9a | **Drop the top-level automatic-caching form** — `body["cache_control"]` never reaches the rebuilt body (KBR-271) | `_convert_native_to_cc_format` | The upstream wire is not native Messages — the same trigger M16 carries | The M9 twin of M16's fourth path (KBR-263 / G38): at the KBR-271 measurement the rebuild built its result from named keys and the Anthropic automatic-caching form (projected to `envelope.extra[cache_control]`, §3.3.1) was dropped by omission on the retried wire. **KBR-296** carried the value onto the internal `_cache_control` carriage and `AnthropicAdapter.translate_to_upstream` restores it verbatim — the retried WIRE now carries the form; the row remains the register's record of the KBR-271 measurement, and the claim is dormant on any route where the restore runs. Pinned at the wire by the KBR-200 CB-3 suite's `top_level` site. **Anticipatory in P28's sense** — the fallback is reachable only on the native route (every call site gates on `_native_messages_request`), so this ROUTE-kind trigger is false on the only path that reaches the site today; the trigger is M16-family symmetry, and the alternative (M9's own RESPONSE-kind trigger) would force `conditional=True` and owe a §3.3.2 complement no corpus entry can author. Claim matching ignores sites, so M16 claims the same address whenever its trigger is met; this row is the register's record of the M9 site's drop, not extra oracle coverage. |
| M9b | **Drop every block-level cache breakpoint** — tool declarations rebuilt from `name`/`description`/`input_schema` only, assistant text joined to a string, `tool_use` rebuilt as a `tool_calls` entry, `tool_result.content` flattened to a string *before* any carriage (KBR-271) | `_convert_native_to_cc_format` | The upstream wire is not native Messages — the same trigger M16 carries | The M9 twin of M16's three carrier paths (KBR-263 / G38): at the KBR-271 measurement the rebuild flattened every block it touched, so a breakpoint on a tool declaration or on any content part was gone on the retried wire. **KBR-296** carried each through — per-tool breakpoints on the name-keyed `_tool_cache_controls` carriage, part-level breakpoints on the CC parts themselves, a last-marked-wins breakpoint on the joined assistant text (`_cache_control`), per-`tool_use` breakpoints on `_tool_call_cache_controls`, and `tool_result.content` now forwarded verbatim (restoring hop 1's KBR-198/KBR-199 nested preservation) — so the retried WIRE carries every carrier the rebuild can express; the row remains the register's record of the KBR-271 measurement, and the claim is dormant on any route where the restore runs. Pinned at the wire by the CB-3 suite's `tool`, `image`, `user_text`, `assistant_text`, `tool_use`, `tool_result` and `tool_result_nested` sites. M9a's anticipatory/M16-overlap caveats apply here identically. Deliberately outside the paths: `system` survives by carriage on `zai_anthropic`/`custom_anthropic` (no drop, no row), and a nested `tool_result` *block* residualises before register matching (M16's "outside this row's paths by design") — both, plus the `minimax_token` system drop this row's trigger cannot reach, are on §9.2 G43. |
...
| M27 | **Drop a top-level `top_k` on the translated (non-native) Chat-Completions-shaped route** — KBR-178's carry mints Anthropic `top_k` on the internal key `_top_k` (`translator.py:439-440`); only the three Messages-routed Anthropic-family adapters (`anthropic`, `minimax_token`, `opencode_go`) restore it, so the other **18** translated adapters drop it by design (`pydantic`-style list form is a Chat Completions no-op) | `MessagesTranslator.translate_request` (`kitty/bridge/messages/translator.py`) | The inbound Anthropic body carries `top_k` | KBR-55 / **G28** (KBR-178; KBR-184 re-derivation). **Unconditional in P13's sense** — fires wherever the field is present, so owes no §3.3.2 assertion-2 complement: the off-state (no `top_k` on the inbound) is also the row-absent state, so no corpus complement can prove anything the trigger-absent case does not already prove. The `conversation.sampling[top_k]` anchor is the narrowest the Anthropic Messages reader projects (`reader_anthropic_messages._SAMPLING_KEYS` at `reader_anthropic_messages.py:84`); P13's bare `conversation.sampling` would over-claim on `openai_subscription`'s CC-origin path, but the specificity rule (`oracle._conditional_violations`) keys on the narrower anchor and P13 is unconditional so assertion 2's attribution never fires on either row. **Scope** = `register._TOP_K_DROPPED_SCOPE` (18 adapters, derived at module import); the naive arithmetic `21 − 5` is wrong because two of `_ANTHROPIC_FAMILY`'s five entries are hardcoded native and never run the translator. **KBR-178.** |
| M28 | **Omit an empty inbound `stop_sequences`** — the carry's truthy-only guard at `translator.py:431-433` (`if stop_sequences: result["stop"] = stop_sequences`) deliberately drops an empty list, because `StopConfiguration.minItems: 1` declares `stop: []` a schema-invalid Chat Completions body | `MessagesTranslator.translate_request` (`kitty/bridge/messages/translator.py`) | The inbound Anthropic body carries an empty `stop_sequences` | KBR-55 / **G29** (KBR-178; KBR-184 re-derivation). **Conditional** because the *non-empty* value is the complement: a non-empty list is carried as-is, so §3.3.2 assertion 2 owes a corpus entry proving the omission's absence. **The trigger case + §3.3.2 complement are this PR's own** — `m28_empty_stop_sequences_trigger.{json,body}` (a body with `stop_sequences: []`) and `m28_empty_stop_sequences_complement.{json,body}` (a body with `stop_sequences: ["END"]`) under `tests/corpus/`, plus the `TestTheKbr55Td5CorpusEntries::test_m28_has_one_trigger_and_one_complement` guard in `test_corpus.py`. (A prior draft of this cell said the entries were `p34*` / `p35_tool_choice_omitted_*`; those entries' `triggers_met` / `triggers_absent` lists name P34 / P35's triggers, not M28's — the reviewer's critical finding.) The Anthropic Messages reader projects `stop_sequences` onto `conversation.sampling[stop]` with no emptiness guard, so an inbound `stop_sequences: []` projects as `conversation.sampling[stop] = []` while the upstream projection has none — that is the row's delta. **Scope** = the same 18 adapters as M27. |
| M29 | **Rewrite a string-form Chat Completions `stop` into the single-item list** | `BridgeServer._normalize_cc_stop` (`kitty/bridge/server.py:1054`), at the `/v1/chat/completions` ingress before the body forks | Always | KBR-55 / **G30** (KBR-178; KBR-184 re-derivation). OpenAI's `StopConfiguration` declares `stop` as `oneOf` a string or an array of one to four strings; every wire kitty writes downstream takes only the array form. An array-form body meets the row as a no-op, so §3.3.2 assertion 2 does not apply. **Both forms project to one `Conversation`**, so the row takes §3.3.1a's `NOT_PROJECTABLE` escape (M15's exact posture) — this **binds T-A2 / KBR-34**: it must read `stop: "END"` and `stop: ["END"]` into the identical `Request`, else a reader would project two equivalent bodies apart, the failure M15 names as the void the escape guards against. **Scope** = `(ALL_PROVIDERS,)` because the `_normalize_cc_stop` seam is on the Chat-Completions ingress route, not an adapter (KBR-139 reachability of the site). **§9.2's stale text named "Row M17" for this row** — KBR-232 reserved M17 for the thinking strip and that is the actual M17; M29 is the landed id. |
| M30 | **Drop an agent's `context_management` on the translated Messages route** — hop 1 never reads the field, so it never reaches any CC wire | `MessagesTranslator.translate_request` (`kitty/bridge/messages/translator.py`) | Provider does not declare `use_native_messages` | KBR-309. Anthropic's `context_management` is a **beta** field — the documented endpoint accepts it only when the request carries `anthropic-beta: context-management-2025-06-27` (fetched 2026-09-24). The bridge builds upstream headers from scratch and forwards no inbound agent header (§4.2 C1), so a KBR-224-style restore would 400 at the upstream without beta-header carriage. The drop is the design posture, not a bug; restoring it is a separate product decision gated on beta-header support. P23 lists the same wire key at the same address, but P23's policy point is the `openai_subscription` Responses allowlist; this row names the Messages→CC translation's own policy point. Scope = `_TRANSLATED_MESSAGES_ADAPTERS` (M16's scope) |
| M31 | **Collapse a multi-text-part user turn into a joined string** — `build_user_content_message` folds every text part after the first into the first's text and drops the rest (KBR-222 byte-identity) | `build_user_content_message` + `MessagesTranslator.translate_request` (`kitty/bridge/messages/translator.py`) | Provider does not declare `use_native_messages` | KBR-309. **The join only fires when no part carries a `cache_control`** — KBR-308 expanded `carry_cache_control=True` from the M9 fallback (KBR-296) to hop 1, so a multi-text-part user turn with a breakpoint on any part keeps the parts as a list and skips the join entirely (the cache_control rides the CC part with it; M16 claims the field-level path). On the corpus-driven slice's passing entries (`plain_turn`, `effort_configured`) every multi-text-part user turn already carries a breakpoint, so the join's surface deltas (`parts[*].text` and `parts[*]`) do not surface today. The row stays live as the register's record of the join's behavior for future corpus entries — the non-cache_control case is the KBR-222 / KBR-258 / KBR-263 product position (register rows, not a `carry_cache_control` expansion). On the non-cache_control path the captured CC body carries one text part whose text differs from the inbound's first text part (`conversation.turns[*].parts[*].text`) and whose collection is one shorter (`conversation.turns[*].parts[*]`); `cache_control` on the dropped parts is itself a loss — M16 claims the field-level drop; M31 claims the part-level collapse that consumes the field. ⚠️ The bare `parts[*]` anchor would also claim a *deleted* part (§3.3.1's own falsification case); the trade-off is recorded so a future reader sees the deliberate cost. M3 / M4 / M8 / M17 also use the same bare-part anchor with conditional triggers; ``_claim_matching`` filters by ``row.trigger in triggers_met`` *before* co-claim, so M31's always-met trigger does not collide with those rows when theirs are unmet, and adds co-claimers when theirs are met (no deferral rule applies in ``_claim_matching``, unlike ``_conditional_violations``'s proper-prefix examples in `oracle.py:1132-1149`). Scope = `_TRANSLATED_MESSAGES_ADAPTERS` |
...
| P18 | Remove `modelId` and `stream` from the Converse payload | `BedrockAdapter._bedrock_body` (`kitty/providers/bedrock.py`) | Always, on `bedrock` | boto3 takes the model id as a call argument and selects streaming by choosing `converse` vs `converse_stream`, so both must leave the body. Applied **in the pure builder, after `translate_to_upstream`** (KBR-89 / T-H2 extracted it from the two transport methods so the pops reach an L1 selection; the transports splat `modelId=<model_id>, **body` verbatim). |
| P19 | Overwrite `stream` | `OllamaCloudAdapter._ollama_body` (`kitty/providers/ollama_cloud.py`) | Always, on `ollama_cloud` | The builder, not the caller, decides which Ollama endpoint mode is used (`make_request` passes `streaming=False`, `stream_request` `True`). Applied **in the pure builder, after `translate_to_upstream`** has already set it from the request (KBR-90 / T-H5 extracted the overwrite from the two transport methods so it reaches an L1 selection; the transports post the returned body verbatim). |
| P26 | **Drop a top-level `cache_control` on the CC-origin translated route** — the Anthropic adapter family never reads the root-level key, so it does not reach the wire | `AnthropicAdapter.translate_to_upstream` (`kitty/providers/anthropic.py`) | The upstream wire is not native Messages — same trigger M2/M16 carry | KBR-258, G37's CC-origin half. The `envelope.extra[cache_control]` projection exists today on the Anthropic Messages reader (G38's remedy records the Messages-route twin); the Chat Completions reader does not yet consume the key into this address, so today such a body residualises and fails the run first — a named, honest failure. The row claims the address the moment the reader grows the slot (the G38 precedent: the row lands before the oracle can drive the body). Five adapters (`anthropic`, `minimax_token`, `zai_coding`, `custom_anthropic`, `opencode_go`) are covered by the one base-class site: the three subclasses short-circuit on `_native_messages_request` and otherwise delegate to `super().translate_to_upstream`, the drops KBR-199 measured happen on the translated (non-native) branch only, and `opencode_go` is the derived fifth — KBR-258 measured four, but its Messages-routed models call `AnthropicAdapter.translate_to_upstream(self, …)` explicitly (`opencode.py:882`), so the same sites run there. The scope column (KBR-139) and the register data both carry five. `conditional=False` per KBR-186 (ROUTE triggers are not corpus-variable) and the `test_rows_sharing_a_trigger_agree_on_whether_it_is_conditional` guard (which already binds M2/M16/M20–M25 to `False` on this trigger). The native passthrough carrying the breakpoint is the observational complement, proven as product behaviour in epic KBR-197, not a corpus entry — the ticket's loose "each row owes a complement corpus entry" prose is reconciled in KBR-258's Jira comment. **KBR-308 closed the drop on the CC-origin path**: the adapter reads a raw CC body's `cache_control` as a fallback when the KBR-296 top-level carriage is absent (carriage wins — DQ-B), restoring it verbatim onto the rebuilt wire. The CB-2 case `top-level-kept` pins the full-value (`{"type": "ephemeral", "ttl": "1h"}`) survival. **KBR-167**; the product-behaviour suite for the same defect is epic KBR-197 |
| P27 | **Drop a system content-part `cache_control` on the CC-origin translated route** — the system-extraction loop joins system blocks into one string and discards breakpoints as it goes | `AnthropicAdapter.translate_to_upstream` (`kitty/providers/anthropic.py`) | The upstream wire is not native Messages — same trigger P26/M2/M16 carry | KBR-258, G37's CC-origin half. The Chat Completions reader projects a system content-part breakpoint onto `conversation.system[*].cache_control` today (KBR-34), so this row is **claimable now** — a body carrying one and routed through the Anthropic family would otherwise report an unclaimed I1 breach. Same site, trigger and conditionality as P26; same M16 precedent on the Messages twin (`system_path(*, cache_control)`); same scope note on the five adapters. `forwards_thinking_signature` does not change this: `_anthropic_system` is a Messages-ingress carriage stripped on the CC route by P1, so the join stands on every adapter. **KBR-308 closed the drop on signature-binding adapters**: the system rebuild now emits a blocks-form `system` (each text block carrying its marker) when any system part carries a marker AND `self.forwards_thinking_signature` is True — the same gate the `_anthropic_system` restore uses. On `minimax_token` and `opencode_go`'s Messages-routed models (`forwards_thinking_signature=False`; MiniMax rejects `cache_control` on system blocks outright, `minimax_token.py:29-30`) the join stands and the G43 scope-out is preserved on the CC-origin path too — the new CB-2 pin `test_system_content_part_drop_is_preserved_on_minimax_token` asserts the joined-string drop and zero wire markers on that route |
| P28 | **Drop a user-message object-level `cache_control` on the CC-origin translated route** — the outer message loop reads each `{"role": …, "content": …}` for role and content only | `AnthropicAdapter.translate_to_upstream` (`kitty/providers/anthropic.py`) | The upstream wire is not native Messages — same trigger P26/P27/M2/M16 carry | KBR-258, G37's CC-origin half. Site covers the user-message and tool-message object case; the assistant-message object case rides P29's site — the oracle matches paths, not sites, so the address is claimed either way. **The kept half — a breakpoint on a user content part and on tool-message content (moved inside the `tool_result`) — is not this row's, and carries no row anywhere in this five-row set**: the adapter family preserves both on the CC route (KBR-199's measurement), the same boundary M16 draws on the Messages twin. The object-level rows (P28, P29) are anticipatory today — the Chat Completions reader residualises a message-dict-level `cache_control`, so such a body fails the run first, the honest named failure; the row lands now, before the reader grows the slot, on the G38 precedent. Anchor contingency: the row assumes the reader projects the message-object `cache_control` onto a Part rather than onto `Turn` itself (`conversation.turns[*].cache_control`, which §3.3.1a does not define today); if a future reader lands the slot on Turn, the anchor must move to match. Same trigger and conditionality as P26; same scope note on the five adapters. **KBR-308 closed the tool-message-object half**: `_tool_result_block` now reads a raw `messages[*].cache_control` as a fallback when the message-level `_cache_control` carriage is absent (DQ-B), attaching it onto the rebuilt Anthropic `tool_result` block. The CB-2 case `tool-message-object-kept` pins the full-value survival. **The user-message-object half stays live — OD1 deferral (PO 2026-09-24)**: Anthropic's wire has no message-level slot, no published CC dialect defines the shape, and any placement would invent a block choice with no fidelity basis. The CB-2 case `user-message-object-deferred-dropped` pins the deferral deliberately |
| P29 | **Drop an assistant-message object-level `cache_control` and a tool_call-level `cache_control` on the CC-origin translated route** — `_translate_assistant_msg` rebuilds the assistant message and its `tool_calls` blocks into Anthropic `tool_use` blocks | `AnthropicAdapter._translate_assistant_msg` (`kitty/providers/anthropic.py`) | The upstream wire is not native Messages — same trigger P26/P27/P28/M2/M16 carry | KBR-258, G37's CC-origin half. Anchored at the same path as P28 because both project to a Part (the tool_use is a Part); distinguishable by site, the axis `test_no_two_rows_are_indistinguishable` explicitly allows (P3/P4 precedent). Same anchor contingency as P28; anticipatory today for the same reason (the reader residualises a tool-call-level `cache_control`), on the G38 precedent. Same trigger and conditionality as P26; same scope note on the five adapters. **KBR-308 closed both drops on the CC-origin path**: `_translate_assistant_msg` now reads a raw `tool_calls[i].cache_control` and an assistant-message-object `messages[i].cache_control` as fallbacks when the index-keyed / message-level carriages are absent (DQ-B), attaching them onto the rebuilt text / `tool_use` blocks. The CB-2 cases `assistant-message-object-kept` and `tool-call-kept` pin the full-value survival at both sites |
| P30 | **Drop a tool-declaration `cache_control` on the CC-origin translated route** — `_translate_tools` rebuilds every tool declaration as `{name, description, input_schema}` | `AnthropicAdapter._translate_tools` (`kitty/providers/anthropic.py`) | The upstream wire is not native Messages — same trigger P26/P27/P28/P29/M2/M16 carry | KBR-258, G37's CC-origin half. The Chat Completions reader projects a tool-decl breakpoint onto `conversation.tools[<name>].cache_control` today (KBR-34), so this row is **claimable now** — a body carrying one and routed through the Anthropic family would otherwise report an unclaimed I1 breach. The Messages-route twin of this drop is M16's tool-decl path. Same trigger and conditionality as P26; same scope note on the five adapters. **KBR-308 closed the drop on the CC-origin path**: `_translate_tools` now reads a raw `tools[i].cache_control` as a fallback when the name-keyed `_tool_cache_controls` carriage lacks the entry (DQ-B), attaching it onto the rebuilt Anthropic tool declaration. The CB-2 case `tool-declaration-kept` pins the full-value survival |
...
| P43 | **Drop the six KBR-301 sampling keys on the rebuild-trio** — `n`, `seed`, `presence_penalty`, `frequency_penalty`, `logprobs`, `top_logprobs` | `AnthropicAdapter.translate_to_upstream` (`kitty/providers/anthropic.py`; its four delegates — `zai_coding`, `custom_anthropic`, `minimax_token`, `opencode_go` on Messages-routed models — reach the base-class body), `BedrockAdapter.translate_to_upstream` (`kitty/providers/bedrock.py`), `OllamaCloudAdapter.translate_to_upstream` (`kitty/providers/ollama_cloud.py`) | Always | KBR-305. Hop-2 leg of the KBR-301 residue named in G44. The six reach the CC body at hop 1 and the verbatim forwarders ship them, but the three rebuild-from-allowlist adapters read only what their destination wire accepts: Anthropic Messages accepts none of the six (`platform.claude.com/docs/en/api/messages`, 2026-09-23); Bedrock `InferenceConfiguration` is exactly `{maxTokens, temperature, topP, stopSequences}`; Ollama `ChatRequest` + `Options` accepts five of the six (`seed`/`presence_penalty`/`frequency_penalty` under `options`; `logprobs` and `top_logprobs` top-level) but has no `n`/`num_choices` field anywhere on Ollama's Go source (`ollama/api/types.go`, 2026-09-23). **Split per adapter**: on Anthropic family + Bedrock all six are dropped; on Ollama Cloud only `n` is dropped, the other five are carried. ⚠️ **Departure from the P31/P32 "paths must be true of every site" rule** (the rule forces a split, one row per adapter family, the P31/P32 shape): on `ollama_cloud` the row's five carried-key claims are dormant. The five carries live in the mutmut scope (`TEST_SUITE.md` §6.1, `kitty.providers.* translate_to_upstream`), so the L1 carry suite is the discriminating guard on that route; the owner confirmed the one-row shape on 2026-09-23. Each path derived from `c.SAMPLING_KEYS` via `c.sampling_path` (closed fifteen, `contract.py:1038-1056`); the exact six-key path tuple is pinned by the new assertion in `test_register.py::test_row_ids_are_unique` (`register_disagreements` compares ids/conditionality/order, not path content), so a data-side drift turns that assertion red, and the markdown row's path list is held by review — a future widening is a deliberate edit to both halves rather than a silent one. |
...
what §3.3.2 assertion 2 tests, with the trigger complements §3.3.4 requires. M1, M2, M9a, M9b, M10, M14, M15, M16, P1,
M20, M21, M22, M23, M24, M25, M26, M27, M29, M30, M31,
P6, P9a, P9b, P9c, P9e, P9f, P9g, P9h, P10, P11, P12, P13, P14, P15, P16, P17, P18, P19, P20, P21, P23, P24, P26, P27, P28, P29, P30, P31, P32, P36, P37, P38 and P43 are
...
(P13–P17); on `ollama_cloud` the hook builds the body and the pure builder `_ollama_body` (extracted from the
two transports by KBR-90 / T-H5, which the transports call) applies the P19 overwrite;
...
| provider aiohttp | `ollama_cloud` | Built by `translate_to_upstream`, then **mutated** by `OllamaCloudAdapter._ollama_body` (P19; the transports post the returned body verbatim). Capture after the mutation. |
...
**The schema is the six fields plan §3 names, plus two.** `id`, `site`, `trigger`, `paths`,
`conditional`, `design_ref` — `not_projectable_reason`, required exactly when `paths` carries
the §3.3.1a escape, because an escape without a reason is a row nothing can falsify — and
`scope` (KBR-139), required on every row with no default: the provider keys the row is reachable
on, per the paragraph below.
...
**Scope is carried, and it names reachability of the site — not survival to the capture
boundary.** §6.2.3's completeness guard and T-D8 both need to know, per adapter, which rows are
reachable, so every row carries a `scope` tuple: the provider registry keys on which at least one
request path executes the row's site, or the single sentinel `*` (`register.ALL_PROVIDERS`) when
every adapter can. Reading that off the site's class is wrong twice — P8's site is
`ProviderAdapter._inject_empty_reasoning_content`, a **base class** method that reads as all 23
adapters while only four call it (`kimi`, `custom_openai`, `zai_regular`, `zai_coding_cc`); and
P5a–d's site is `AnthropicAdapter.translate_to_upstream`, which four subclasses override *and
conditionally delegate back to*, so the row is also reachable on `custom_anthropic`, `zai_coding`,
`minimax_token` and `opencode_go` — which no static rule over the class hierarchy finds. The scope
was therefore **authored by reading each override and delegation chain** and is held honest two
ways: the guard (`register.scope_problems`) checks every non-sentinel key against the registry
read by AST, never by import (§3.3.1's independent-oracle rule), and pins each row's scope to the
files its own sites name; and a wrong entry that survives the guard fails a real oracle run, which
is the falsifiability plan §1.4 demanded and which the oracle (KBR-51) now supplies — the reason
this column could not be authored in T-W3.

**Reachability is deliberately not survival (KBR-160).** KBR-160 caught M1 true at its site,
trigger, conditionality and scope while a consumer one layer down discarded the row's effect
before the wire. "Reachable" means the site executes on some request path through the adapter; it
does not mean the effect reaches §3.2.3's capture boundary, and scope cannot say that — a consumer
between site and boundary can silently undo a row. Survival is asserted at the boundary (the
KBR-160 pattern); generalising that assertion to the bridge-level rows × the three custom-transport
adapters (`openai_subscription`, `bedrock`, `ollama_cloud`) is a recorded follow-up, not a scope
entry. The P9e structural check proposed in PR #133's review (require a row's review to cover a
subclass that later overrides an inherited site) is likewise **deferred**: it guards a
markdown-prose→review axis orthogonal to scope data, and the per-row delegation comments plus the
site↔scope guard carry the reachable-delegation facts.

Three derived facts are worth naming because they are not obvious from the class hierarchy alone:
the M9 fallback rows are reachable only on the three adapters whose `use_native_messages` is true
(`custom_anthropic`, `minimax_token`, `zai_coding` — `anthropic` inherits `False`); the
Messages-translator rows (M16, M26, P34, P35) are unreachable on the two adapters that hardcode
native (`custom_anthropic`, `zai_coding`), because the /v1/messages handler skips the translator
there; and `P4` excludes `openai_subscription`, whose custom transport never reaches
`translate_to_upstream` (§6.2.3). The full derivation is recorded in
`.system_design/steps/kbr139_register_scope.md`.
...
| Data ⇄ the provider registry — every scope key resolves, and every site-named adapter is in scope | L2 | **T-W3** (KBR-139) | The AST of `providers/registry.py` plus the register data. No sockets |
...
and it needs a recorder and an oracle. Twelve such omissions were known and filed before this PR
landed: G26
...
1. **Closing a `curl_cffi` session under a live SSE stream is the one shape that has crashed.**
   ...
   **0.16.3**, the version resolved here — `pyproject.toml` bounds the range at `>=0.15,<0.17`
   (KBR-283), so drift *within* the bounded families stays possible and this is measured rather
   than guaranteed. More
...
broken. KBR-86 closed the per-variable and scheme-scoping halves: the matching-`NO_PROXY`
precedence is pinned per casing (the both-set form only ever proved the lowercase-read path —
urllib's lowercase preference shadows the uppercase variable), a non-matching `NO_PROXY` control
pins that the matching probe means *matching*, each consulted casing of `HTTPS_PROXY`/
`https_proxy` is individually falsified (mapping absent, dead ambient, drive must die against
the dead proxy with the failure text naming its host), and `HTTP_PROXY`/`http_proxy`/
`ALL_PROXY`/`all_proxy` are pinned **not** to steer an `https://` request even with the mapping
absent (botocore reads only the scheme key — the `all` catch-all urllib carries is never read
back). Pinned at `tests/harness/test_botocore_transport_contract.py`, mirroring the curl_cffi
...
- **Scope — narrow, but it must include the code the rationale is about.** An earlier draft
  ...
  | `kitty.providers.ollama_cloud.OllamaCloudAdapter._ollama_body*` (KBR-90, T-H5) | P19 — the `/api/chat` body's `stream` overwrite. Lived inside the network methods before this row existed; the refactor pulled the body shaping into a pure builder so mutmut could reach it. Same per-adapter shape as the row above — a sibling, not a `provider_hooks` member (unlike P18's defensive pop, the overwrite is what the transport really applies, so the builder is parametrised on a `streaming` flag and the kill is real) |
...
- **P18 and P19 needed a refactor before they could be mutation-tested.** Both lived inside
  `make_request` / `stream_request`, which open network connections, so `mutmut` could not reach them
  from an L1 selection. The payload shaping was extracted into a pure builder — `_bedrock_body(...)`,
  ...
  test, not the test that is hard to write. **P18's half landed with KBR-89 (T-H2)** —
  `_bedrock_body(...)` exists, is a scope row (see the table above), and is reached by
  `TestBedrockBody` in `tests/test_provider_bedrock.py`. **P19's half landed with KBR-90 (T-H5)** —
  `_ollama_body(...)` exists, is a scope row (see the table above), is reached by
  `TestOllamaCloudBody` in `tests/test_provider_ollama_cloud.py`, and is parametrised on a
  `streaming` flag because, unlike bedrock's defensive pop, the overwrite is what the transport
  really applies (`make_request` passes `False`, `stream_request` `True`).
...
| Guard | Asserts |
|---|---|
| **Register completeness — shape diff at the wire** | For every adapter × representative model × transport, capture the body at the §3.2.3 boundary and assert the projected delta from the input is exactly the union of that adapter's register rows whose triggers the input met — "that adapter's" reads off the row's `scope` column (KBR-139): the rows whose scope names the adapter, the `*` sentinel matching every one. **One fixed request is not sufficient** — each conditional row needs a trigger case and a complement case (§3.3.4), and adapters that route by model need one input per route. |
...
| **No test module imports the `tests` package** | No `tests/**/*.py` module contains an absolute `import tests` / `import tests.X` or `from tests import …` at module top level or inside any nested `def`, `async def`, or `class` body. Landed with **KBR-280** as `tests/test_no_tests_package_imports.py`; widened to nested scopes with **KBR-282**. **The trap:** CI's fast-gate invokes bare `pytest` (`.github/workflows/tests.yml`), whose import resolution differs from a developer's `python -m pytest` (which prepends cwd); `tests/` is a rootdir without `__init__.py`, so `import tests.X` resolves under one invocation style and raises `ModuleNotFoundError: No module named 'tests'` under the other — one line taking down the whole six-job matrix. KBR-84 (PR #214) paid a full CI round for exactly this on a contract test's self-guard. The walk is an explicit allowlist of the three Python scope statements (`FunctionDef`, `AsyncFunctionDef`, `ClassDef`), recursing to arbitrary depth; every other statement-bearing node is a leaf. **KBR-280 scoped the guard to direct top-level `import` / `from-import` statements** (the KBR-84 failure shape) and deferred widening as "a separate, conscious decision"; **KBR-282 is that decision** — the function-level negative control was consciously rewritten into a positive control, as its own docstring's tripwire required. Three adjacent shapes are deliberately out of scope: (a) `if TYPE_CHECKING:`-guarded imports, which never execute at runtime under either style and would be a false positive; (b) runtime-guarded wrappers — the failure-avoiding spellings `try: import tests.X / except ModuleNotFoundError: pass`, `if False:`, `while False:`, `with contextlib.suppress(ModuleNotFoundError):`, and their `async`/`*Star` siblings — where the author has explicitly handled the failure mode — not the accidental KBR-84 shape (a plain `with` whose manager does not suppress is skipped by the same leaf rule but is an executable position the walker does not visit, like the `for`/`match` limits below); the KBR-280 contract pinned this exemption at top level and KBR-282 extends it to function depth, pinned by a parameterised negative control over all four base shapes inside a function; (c) dynamic `importlib.import_module("tests.X")`, which an import-statement matcher cannot see (sibling rows state limits of this shape explicitly). Out-of-scope shapes (a) and (b) are pinned by negative-control tests; (c) is a documented limit. The matcher is a shared helper called by both the live scan and the synthetic falsification probes. **No live positive control exists** — the KBR-84 fix removed the only real offender — so the positive controls are the synthetic inline cases, at top level and at each nested position the walker descends into (function, async function, class body, method, function-in-function, class-in-class), mirroring `tests/bridge/test_vendor_token_guard.py`'s historical-M13 pattern; the live-scan self-guard asserts the walk covers a non-empty population including the guard file itself. **Why an AST guard, not `pythonpath = ["."]`:** the blanket fix would expose both `harness` (pytest's prepended rootdir) and `tests.harness` as importable — a double-import hazard where one module holds two identities and tests relying on either form can silently mask or break. The guard rejects the pattern without changing import resolution for anything else. **Further stated limits (KBR-282):** whole-node skip on guard statements covers their `orelse`/`finalbody` arms, so an unconditionally executing import inside `finally:`/`else:` is not flagged — the skip rationale is "the whole statement is a guard", not "inspect every arm", and descent into guard arms would re-implement a runtime-guard detector inside the matcher; `for`/`async for`/`while`/`match` bodies are not descended into and a scope statement itself wrapped in a guard (`if flag: def f(): import tests.X`) is invisible, because the ticket enumerates only the three scope types and the next widening must add them explicitly and re-state the runtime-guard contract uniformly. **Stated limit:** the guard's rationale assumes `tests/` remains a rootdir without `__init__.py`; adding one would break the bare-import convention (`import layers`, `import internal_key_scan`) that this guard's rationale rests on, and would invalidate the KBR-84 hazard model. |
| **Project CLAUDE.md review-resolution discipline** | The repo-root `CLAUDE.md` is the instruction every Claude Code session acts on, yet nothing imports it, so its presence, tracking, ignore-status, and the four mandated review-resolution behaviours would otherwise drift silently. Landed with **KBR-286** as `tests/test_project_claude_md_review_discipline.py`. Presence is `Path.exists` + `git ls-files --error-unmatch`; ignore-status is `git check-ignore --no-index` with exit code 1 as the only acceptable state — **`--no-index` is load-bearing**: git's default short-circuits tracked files out of the ignore consultation entirely, so a plain `check-ignore` on the tracked `CLAUDE.md` would exit 1 even with `/CLAUDE.md` re-added to `.gitignore`, and the re-ignore scenario the guard exists for would pass silently (caught by the automated review round on the KBR-286 PR); exit 128 (git failure) is branched into a separate `pytest.fail` so a broken git sends the reader at the right failure rather than at a phantom ignore pattern (caught by the same round). The four behaviours are pinned by stable anchors inside the review-resolution section only, whitespace-normalised so a re-wrap cannot break them, each anchor unique to its rule's wording (so sub-clause weakening trips the guard, not just whole-rule deletion — the four-clause coverage was tested live by removing each clause's anchor phrase in turn); the file is read with `encoding="utf-8"` (KBR-266 cp1252 trap). Self-guarded per the §6.2 rule: each clause is independently tested, and an in-suite assertion (`test_the_section_extraction_does_not_leak_preamble_text`) holds the section-scoping property under drift — a silent widening of `_review_section()` would otherwise pass every clause test while satisfying the anchors from outside the section. |
...
|---|---|---|
| `aiohttp` | `>=3.11,<3.14` | A session built with `proxy=`/`proxy_auth=` proxies, and a per-request `proxy=None` cannot escape it. `_build_client_session` sets the proxy at session level precisely so no call site can forget it, and `_session_for` depends on a request being unable to opt out. **Landed (KBR-84): `tests/test_aiohttp_transport_contract.py`.** |
| `curl_cffi` | `>=0.15,<0.17` — **bounded (KBR-283, product-owner decision 2026-09-18)** | **Bounded:** curl_cffi's 0.x minors carry breaking changes (`Response.cookies` retyped; `AsyncSession.close` became a coroutine; the plain-http CONNECT preference flipped across minors), the 0.16.3 Windows build already showed same-release platform drift on `HTTP_PROXY`, and the leg this transport carries is the OAuth refresh — refresh tokens and API keys. The floor is the oldest minor the product owner selected to support — a supported-range statement, not a measured-on-floor guarantee — and the chosen mechanism is **both** a ceiling and the contract suite: neither alone fences drift in the leg that matters (§5.5). A deliberate upgrade refreshes the range and the character-exact pin guard (`tests/test_pypi_packaging.py::TestVersionConsistency`) in one PR and runs `tests/test_curl_cffi_transport_contract.py` against the new resolution; `uv.lock` is gitignored, so the declaration is the only durable pin. **Contract suite landed (KBR-161 + KBR-85): `tests/test_curl_cffi_transport_contract.py`.** `proxies=` is honoured; `data=dict` form-encodes (the token grants depend on it); an explicit `User-Agent` beats the one `impersonate=` injects (the whole of KBR-161's fix depends on it); the impersonation target still exists. On the environment, the measured answer is the *opposite* of what this row assumed: a matching ambient `NO_PROXY` **defeats** `proxies=`, and only `CURLOPT_NOPROXY` overrides it — both directions pinned, since a future release reversing either must turn red rather than silently change containment. **The same precedence question now extends to the scheme-scoped ambient variables and the scheme-agnostic catch-all (KBR-85)**: `proxies=` beats lowercase `http_proxy` for `http://` URLs (uppercase `HTTP_PROXY` is a documented libcurl CGI-security no-op on Linux/macOS, but the curl_cffi 0.16.3 Windows build honours it — both pinned per-platform so a release that flips either direction turns red) and beats `HTTPS_PROXY` / `https_proxy` / `ALL_PROXY` / `all_proxy` for `https://` URLs — the leg the OpenAI OAuth refresh rides, and the leg an ambient variable would defeat on. `HTTPS_PROXY` is therefore not a separate contract: the curl_cffi-side half of §5.5's ambient-proxy story is now closed for every scheme, on every supported platform. |
| `botocore` | `>=1.34` (declared directly in `pyproject.toml` so a `botocore` bump goes through the lockfile rather than `boto3`'s transitives — KBR-64 closed the original "arrives transitively" complaint) | `Config(proxies=)` is honoured and takes precedence over the environment — a matching `NO_PROXY`/`no_proxy` does not defeat the mapping (pinned **per casing**, parametrized; the both-set form only ever proved the lowercase-read path) and a **non-matching** `NO_PROXY` leaves the mapping alone — and ambient `HTTP(S)_PROXY` and the catch-all set to a dead address are not consulted. Per-variable falsification: with the mapping patched away (`get_egress` → `None`), each casing of `HTTPS_PROXY`/`https_proxy` alone at a dead address kills the drive (non-200, zero harness-proxy CONNECT attempts, failure text naming the dead proxy's host — the discriminator that stops the probe passing vacuously), while `HTTP_PROXY`/`http_proxy`/`ALL_PROXY`/`all_proxy` alone at a dead address leave the drive answering 200 direct (botocore reads only the scheme key; the `all` catch-all urllib carries is never read back) — proving each precedence probe can fail when broken and pinning the scheme scoping in both directions. All pinned at `tests/harness/test_botocore_transport_contract.py`. **Landed ([KBR-64](https://shelpuk.atlassian.net/browse/KBR-64), PR #204); per-casing + per-variable falsification and the scoping negatives landed in [KBR-86](https://shelpuk.atlassian.net/browse/KBR-86).** |
| `keyring` | `>=23.0` | Backend resolution on each supported platform. Resolution varies by platform **by design** — the "no stable neighbour" case recorded below — so the chosen mechanism is the declared floor **plus a contract on the resolution mechanics**, not on live native services: the public API delegates to `keyring.get_keyring()` (pinning it pins where credentials go — the fact `KeyringBackend`'s blind delegation rests on); `PYTHON_KEYRING_BACKEND` selects the backend via `keyring.core.load_env()`; resolution always lands on a `keyring.backends.*` class; the per-platform native class is asserted only where the native service is reachable (the Windows leg asserts — pywin32-ctypes is a base dependency; the macOS arm skips with a stated reason where pyobjc is absent, which the bare pin does not carry; on Linux the `fail.Keyring` fallback is asserted only behind a SecretService-unavailable guard, because a D-Bus box classifies to the chainer); and `keyring.errors.PasswordDeleteError` ⊂ `KeyringError` is the family `KeyringBackend.delete` suppresses. **Landed (KBR-87): `tests/test_keyring_backend_contract.py`.** |
...
| `SIGKILL` mid-`kitty kilo`, then `kitty cleanup` (KBR-268) | The Kilo arm `run_kilo_cleanup` restores `kilo.json` byte-exactly from `~/.config/kitty/kilo-config-backup.json` (KBR-262's `newline=""` contract on both legs) and removes the backup; a **stale** backup (config readable, no kitty markers per `_kilo_kitty_values_present`) is deleted with the config untouched; a missing or unreadable `kilo.json` is resurrected from the backup. Negative control: marker damage with **no** backup is a strict no-op — the accepted residual of the backup-only design (a backupless heuristic could never restore the overwritten `model` key) |
...
| a **required field** of a part — a tool `name`, `fileData.fileUri` | **raise on `name`**: the seven invocation readers (KBR-281 + KBR-292) and the six declaration branches (KBR-295) all raise on absent / empty / non-string tool names; **`fileData.fileUri`** still residualises with the canonical-JSON digest as identity | **Tool-call `name`, four strict readers — settled KBR-281.** Chat Completions (`reader_chat_completions.py:466-470` reply, `499-503` legacy reply, `952-956` request), Gemini (reply `reader_gemini.py:623`, request `reader_gemini.py:1803` — both via the module-shared `_require_tool_call_name`), Anthropic (`reader_anthropic_messages.py:751-756` — the single site shared by both directions), and Ollama (both directions, `_require_tool_call_name` at `reader_ollama.py:1007`) all raise `UnreadableBodyError` on absent / empty / non-string. This inverts the row's general prescription for tool-call names; the per-reader Because cell previously recorded "stricter raises as deliberate deviations" — the deviation has become the settled rule. **The two-reader helper seam.** Gemini and Ollama both route their tool-call name check through `_require_tool_call_name` so the rules grep with `git grep _require_tool_call_name tests/harness/`; Chat Completions and Anthropic raise inline at the call site (their pre-existing inline `or not name` form was tightened in the same pass and is consistent with the helper's spelling). **KBR-292 extension (2026-09-21):** the three readers that KBR-281 left open — Bedrock Converse request (`reader_bedrock_converse.py:1118` — module-level `_require_tool_call_name`), Responses request (`reader_responses.py:276` — module-level `_require_tool_call_name`, shared by both directions), and Responses reply (same `reader_responses.py:276` helper) — now also raise on absent / empty / non-string. Bedrock Converse and both Responses directions route through their own per-module `_require_tool_call_name` helper (mirroring Ollama and Gemini), so `git grep _require_tool_call_name tests/harness/` now lists **four** helper modules: Ollama, Gemini, Bedrock Converse, Responses. All seven tool-call readers now agree on raise, so the per-reader picture this paragraph once recorded — "still residualise / silent on `""` / partial" — is closed. **KBR-295 extension (2026-09-21): declarations.** The six declaration branches — the entry a request puts in `tools: [...]` — close the deliberate sibling KBR-292 recorded and raise on absent / empty / null / non-string too, completing the settlement across all seven wire formats (the seven invocation readers + the six declaration branches). Gemini `FunctionDeclaration` (via `_read_required_name`, `reader_gemini.py:1289` — now a thin aliased-view wrapper over the module's `_require_tool_call_name`) and Responses `FunctionTool` (`_read_tool`'s `function` branch, via the module's `_require_tool_call_name`) replace the residualise-and-project-`ToolDecl(name="")` posture KBR-292 had deliberately left open; Converse `toolSpec` (`_read_tool_specification`, via the module's `_require_tool_call_name`) replaces the omit-the-declaration posture with a raise for the `name` field alone — its `inputSchema` and other required-field failures keep the residualise-and-omit posture, an asymmetry Chat Completions (`reader_chat_completions.py:1276` raise on `function.name` vs the residualised `parameters` leaf) and Anthropic (`reader_anthropic_messages.py:574` raise on `tools[].name` vs the residualised `input_schema` leaf) already carried on their declaration branches, pinned by `TestDeclarationNameRequired::test_input_schema_failure_still_omits_the_declaration`. The design review that landed the widened scope (KBR-295, 2026-09-21) found the same silent-`""` shape on three more declaration branches the ticket text had not named: Chat Completions `function.name` (`reader_chat_completions.py:1276` — type-only raise, `""` slipped through), Anthropic `tools[].name` (`reader_anthropic_messages.py:574` — same type-only raise), and Ollama `tools[].function.name` (`reader_ollama.py:1325` — `_typed_leaf(...) or ""` projected `ToolDecl(name="")` for every bad shape with no raise at all); all three converge on the settled rule in the same pass, so no residual sibling remains. **KBR-299 extension (2026-09-22): the Responses custom / built-in / MCP declaration sub-branches.** The non-function fallback of the Responses reader's `_read_tool` (the branch that labels a declaration by its kind — `str(kind)` for `custom` / built-in, `_mcp_tool_name(label)` / `"mcp"` for `mcp`) admitted `name=""` silently, the last silent site the seven readers + six declaration branches did not cover. The settlement is deliberately narrower than the strict rule: only the empty shape raises (`UnreadableBodyError` at `tools[<i>].name`, same message spelling as `_require_tool_call_name`); absent / null / non-string keep the kind-derived label. The asymmetry's recorded why: the kind-derived label is a *wire-derived identity* — built-in declarations carry no `name` field at all, so absent is the published shape and raising on it would reject legal wire traffic; `mcp`'s identity derives from `server_label`; `custom`'s schema makes `name` required, but the ticket tightens only the `""` shape and leaves the absent posture for a future settlement rather than widening the blast radius. An empty string is different in kind — a wire value that *names nothing* (`contract.decode_arguments`' losslessness argument), which is why it raises even on the `mcp` sub-branch, where the empty field is silently overwritten by the server-derived label today. Six control tests (absent + non-string per shape) pin the permissive posture so a future widening of the guard fails loudly. **KBR-303 extension (2026-09-23): the selection surface, five readers, plus the Responses mcp `server_label` declaration slip.** The same silent-`""` shape lived on `tool_choice`'s by-name selectors across **five** readers — Responses (`_normalise_tool_choice` by-name + mcp branches), Chat Completions (`_read_tool_choice` by-name), Anthropic (`_read_tool_choice` `tool` branch), Bedrock Converse (`_read_tool_choice` `tool` block), and Gemini (`_read_tool_choice`'s `allowedFunctionNames[0]` single-element projection) — each producing the never-legal selection string `"tool:"` (or the trailing-colon forms `"tool:mcp:<label>:"` / `"tool:mcp:"` on the Responses mcp branch) because only the `isinstance(x, str)` type was checked, not the value. The design reviewer's sibling-arms sweep (KBR-303, 2026-09-23) found the same defect class on the four readers the ticket text had not named; all five converge on the settled rule in the same pass. The settlement extends the empty-only raise to the selection surface with the family message spelling, so `git grep "must be a non-empty string" tests/harness/` lists every site; the absent / non-string postures stay deliberately permissive — residualise on Responses' by-name + mcp `server_label` paths, the narrow-to-server projection (`"tool:mcp:<label>"`) on Responses' mcp `name` path, the existing raises on Chat Completions + Anthropic (their non-string checks already raised; only `""` slipped through), and the residualise on Converse + Gemini — pinned by the same control-test pattern KBR-299 shipped, so a future widening of the guard fails loudly. The adjacent Responses **declaration** mcp `server_label=""` slip (`_read_tool`'s mcp arm projecting `ToolDecl(name="mcp:")` via `_mcp_tool_name("")`) closes in the same pass — one field over from KBR-299's `name` settlement, the same never-legal-empty-identity family — with absent / non-string keeping the `"mcp"` fallback posture KBR-299 recorded as deliberate. **General-prescription pointer.** §3.3.1b's flat residualise sentence is the row's default for an unreadable required *value*; the tool `name` is settled here as an exception for the seven invocation readers, the six declaration branches, the Responses custom / built-in / MCP declaration sub-branches (empty shape only), and — since KBR-303 — the `tool_choice` selection surface's empty shape on the five readers that carry a by-name selector plus the Responses declaration mcp `server_label` sub-branch (empty shape only), so a future reader author sees the raises as §7.4.2 row 2, not a §3.3.1b violation. **`fileData.fileUri`** — unchanged. Residualises with the canonical-JSON digest as identity; `Image.__post_init__` enforces XOR (KBR-192), so a missing `fileUri` cannot project `Image(ref=None)`; the part keeps its position with the `opaque_digest` recipe. |
...
| **G16** | **F5 — `OpenCodeGoAdapter` misdeclared its wire shape** — KBR-7 · **CLOSED** | Was a latent defect in the M8 path and a trap for the oracle | Done: the declaration is per-model, both repair sites branch on it, the **hook-level** honesty guard landed with the fix, and the **wire-level** guard landed with **KBR-80 (PR #217, 2026-09-17; sibling file `tests/test_wire_shape_honesty_wire.py`)** | — |
...
| ~~**G43**~~ | ~~**The M9 fallback converter drops cache breakpoints beyond the two carriers its carriage restores** — KBR-271~~ · **CLOSED 2026-09-23** | Was: found by KBR-200's CB-3 suite, which pinned the M9 fallback's wire behaviour (`tests/bridge/test_native_passthrough_cache_breaks.py`); KBR-263's closure recorded the residue for separate triage, and KBR-271 followed up. At the KBR-271 measurement the converter rebuilt the body from named keys, flattening every block it touches: the tool declaration, all content parts (`image`, user/assistant text, `tool_use`), `tool_result.content` — flattened to a string *before* any carriage, defeating the Messages translator's KBR-198/KBR-199 nested preservation — and the top-level automatic-caching form. **KBR-271's register half has landed** (M9a/M9b, same trigger family as their hop-1 twins M16/P26–P30) — the rows are anticipatory in P28's sense, because the fallback is reachable only on the native route (every call site gates on `_native_messages_request`), so the ROUTE-kind trigger cannot be met there today and no corpus entry exercises the fallback; §4.3 C2 would fail such a run before assertion 1. Two drops stay **unclaimed** by design of that trigger: the `system` carrier on `minimax_token` (the carriage lands on `_anthropic_system` verbatim but the adapter's `forwards_thinking_signature=False` means it is never restored — a drop on a *native* route, outside `NON_NATIVE_UPSTREAM_WIRE`'s reach; `zai_anthropic`/`custom_anthropic` restore it, which is why the CB-3 suite counts `system` as preserved) and the nested `tool_result` *block* (residualises before register matching, unaddressable in the §3.3.1a vocabulary — M16's "outside this row's paths by design"). The product cost is the KBR-258/KBR-263 class: a stripped breakpoint re-bills the agent's stable prefix at ~10× the cached rate on every fallback turn | Done: **KBR-296** carried every breakpoint the rebuild can express — top-level `cache_control` on `_cache_control` (Anthropic's automatic-caching form, verbatim), per-tool `cache_control` on the name-keyed `_tool_cache_controls` carriage (P30-aligned), part-level on the CC parts themselves (the adapter's existing restore at `anthropic.py:714, 718-719`), a last-marked-wins breakpoint on the joined assistant text (message-level `_cache_control`), per-`tool_use` breakpoints on `_tool_call_cache_controls`, and `tool_result.content` forwarded verbatim so the nested block's breakpoint rides in place (closing the unclaimed nested-block residue). All carriers survive on every native adapter by attempt-0 parity (the same value already reached the upstream on attempt 0, where native passthrough ships the raw body verbatim). **The `system` carrier on `minimax_token` is scoped deliberately**, not closed by widening: the `_anthropic_system` restore stays coupled to `forwards_thinking_signature` (KBR-228's signature-binding contract), and `src/kitty/providers/minimax_token.py:29-30` documents that MiniMax's endpoint rejects `cache_control` on system blocks outright — widening the restore would 400 the retry. The anticipatory M9a/M9b rows remain as the register's record of the KBR-271 measurement (their claims are dormant on any route where the restore runs); the count pins are unchanged and the row text is amended in lockstep to name the post-fix behaviour. The CB-3 suite (`tests/bridge/test_native_passthrough_cache_breaks.py::_SURVIVES_ON_ADAPTER`) widens from 2 sites to 10 on `zai_anthropic`/`custom_anthropic` and from 1 to 9 on `minimax_token` (the deliberate `system` scope-out). | — |
| **G44** | **Five format-specific `generationConfig` control fields are dropped on the Gemini ingress, and the six newly carried sampling keys die at hop 2 on the rebuild families — unregistered** — KBR-301 | Found by KBR-301's sweep (the KBR-213 sibling) and its design review. Gemini v1beta `GenerationConfig` publishes 25 keys, pinned by `tests/harness/test_reader_gemini.py::TestSchemaAgreement` at `SCHEMA_VERSION == "20260910"`. The harness reader projects 11 of them onto canonical Chat Completions sampling spellings (`_SAMPLING_KEYS`, `reader_gemini.py:138-149`) — KBR-213 carried five (`temperature`, `topP`, `maxOutputTokens`, `stopSequences`, `topK`), KBR-301 carries the six remaining (`candidateCount`→`n`, `presencePenalty`, `frequencyPenalty`, `seed`, and the collision pair `responseLogprobs`→`logprobs` / `logprobs`→`top_logprobs`) — and the remaining 14 onto `envelope.extra[<leaf key>]` (`_GENERATION_EXTRA_KEYS`, `reader_gemini.py:163-180`). **First residue — the five named control fields.** Five of the 14 are in KBR-301's filed scope: `responseMimeType` (per-modality output configuration — `"application/json"`, `"text/plain"`, `"audio/wav"`), `responseSchema` (schema constraints), and the three Gemini-specific vendor extensions `thinkingConfig`, `mediaResolution`, `speechConfig`. Chat Completions declares no field any of them maps to — `reader_gemini.py:152-162` records why no fold onto `response_format` exists ("folding any of them onto the Chat Completions union would put a claim into the projection that the wire does not make") — so the production translator (`bridge/gemini/translator.py`) drops all five, and each is an unclaimed `envelope.extra[<wire key>]` delta on every Gemini route the moment T-D5 drives a corpus entry carrying one: the under-claiming direction §3.3.1a calls unrecoverable, G28's shape. **Second residue — the hop-2 sampling drops on the rebuild families.** The six newly carried keys survive only on the verbatim forwarders (the OpenAI family et al.); the rebuild-from-allowlist adapters drop them at hop 2 — `AnthropicAdapter.translate_to_upstream` reads only `model`/`max_tokens`/`stream`/`temperature`/`top_p`/`stop`/`_top_k` from the CC body, `BedrockAdapter` the same rebuild shape, `OllamaCloudAdapter` carries only `temperature`/`top_p`/`top_k` into `options` — so on those routes a Gemini client's `seed`/`n`/`logprobs` request still dies, one hop later than before KBR-301. `openai_subscription` is the exception that is already claimed: its `_cc_only_params` set names all six, and P13/P14's bare `conversation.sampling` anchor covers that route. The rebuild-trio residue is G28's exact class at the same address family (`conversation.sampling[<key>]`) and widens G28's scope when its row lands. **Out of scope here, deliberately recorded:** the other nine `_GENERATION_EXTRA_KEYS` entries (`_responseJsonSchema`, `audioTranscriptionConfig`, `enableAffectiveDialog`, `enableEnhancedCivicAnswers`, `imageConfig`, `responseFormat`, `responseJsonSchema`, `responseModalities`, `translationConfig`) have been in the harness reader since its creation (`12abf4e`, 2026-09-12) and are the same defect class; a separate ticket can widen this row's address list | One bridge-level row naming the five `envelope.extra[<wire key>]` addresses at site `GeminiTranslator.translate_request`, trigger ALWAYS, **unconditional** — these drops fire wherever the wire carries the field, so no §3.3.2 assertion-2 complement is owed, G28's shape. **The rebuild-trio sampling residue is claimed by KBR-305**: a new provider-level sibling row **P43** at the adapter sites (`AnthropicAdapter.translate_to_upstream` + its four delegates, `BedrockAdapter.translate_to_upstream`, `OllamaCloudAdapter.translate_to_upstream`), unconditional, paths the six `sampling_path(n|seed|presence_penalty|frequency_penalty|logprobs|top_logprobs)` addresses, scope `_ANTHROPIC_FAMILY + ("bedrock", "ollama_cloud")`. The remedy taken is the sibling-row shape G44 anticipated ("a sibling row at the same anchor"), not the G28-widening alternative — KBR-305 records the per-adapter split (Anthropic family + Bedrock drop all six; Ollama Cloud carries five, drops only `n`) and the accepted P31/P32-rule departure on the one-row scope. **Deferred deliberately, not overlooked:** only the trigger case needs the T-D5 Gemini corpus (unconditional rows owe no complement), as G28. **Before T-D5** | **1** |
...
| **G42** | **User images and documents on the routes that still drop them, unregistered** — KBR-222 | KBR-222 carries an agent's `image` blocks as CC `image_url` parts and `document` blocks on the `_documents` internal key. `AnthropicAdapter` and its delegates restore both (the tool run and its trailing user message re-join into one user message there, the shape Anthropic's docs prescribe); the OpenAI-family verbatim forwarders ship `image_url` parts natively. The remaining routes keep a deliberate drop no row claims: `ollama_cloud`'s `_flatten_content` flattens a parts list to its text, and `openai_subscription`'s `_cc_to_responses` flattens a parts list to its text, an image-only list yielding no input item at all. KBR-223 closed bedrock's user-branch flatten — the mapper turns parts list into Converse blocks, so `bedrock` ships images now — and the row's bedrock mention is removed. M5's pruning forfeit is recorded in that row. `reader_anthropic_messages` and `reader_responses` already project images on their wires, so the moment the corpus carries one (KBR-45's), the oracle reports a **real** I1 breach on each route in this list — a true positive that is still unclaimed | One row per route's flatten/drop, or claim them in M2's prose per destination as KBR-45 lands. **Before T-D5 drives a corpus entry carrying an image or document** | **1** |
...
| **G11** | Dependency behaviour unpinned; `curl_cffi` unbounded, **botocore undeclared** and the interpreter declared to the minor only | **Partly closed (KBR-64, KBR-283).** The `botocore` declaration now lands in `pyproject.toml` (`botocore>=1.34`), named explicitly in the §6.2.4 row — and the four transport contract tests have landed (KBR-161 `curl_cffi` precedence + no-bypass; KBR-64 botocore precedence + no-bypass). **KBR-283 bounded the `curl_cffi` pin** (`>=0.15,<0.17`, product-owner decision 2026-09-18; a character-exact pin guard in `tests/test_pypi_packaging.py` makes every deliberate upgrade touch the guard in the same PR). Still open: `keyring` lacks a bounded version (the KBR-87 resolution contract has landed and pins the mechanics; the bounded-pin half remains a product-owner decision), the interpreter is still declared to the minor only | Dependency contract tests (§6.2.4) + declare botocore | **3** |
...
per-route confirmation pending), and amended the same day by KBR-247.** The no-finish
empty-stream hole that KBR-235 closed on `/v1/messages` had the same shape on
`_stream_responses` (Codex CLI) and `_stream_gemini` (Gemini CLI): the gate in `_stream_*`
still read `translator.response_was_empty and finish_events`, so an upstream that answered
`200` with zero bytes or `[DONE]`-only passed both conditions' guard and reached the agent
as an empty turn with the backend marked healthy. The KBR-250 fix is the same on both
routes: a no-finish arm `empty_no_finish = not finish_events and not events_emitted`
extends the gate to `or empty_no_finish`, and the D4 exhaustion path writes an SSE error
event into the open `sr`, lets the post-loop synthesize `response.completed` with
`status="incomplete"` (responses) or `write_eof()` without the healthy-mark (gemini), and
returns `sr` with `200 text/event-stream`. The route-specific D4 discriminators carry the
same semantic KBR-235 chose for the messages branch:
`responses_format_error({"code": "empty_response", "message": ...}, seq=N)` producing a
top-level `{type: "error", code: "empty_response", ...}` on the Responses wire;
`{"error": {"code": 502, "message": ..., "reason": "empty_response"}}` serialized as
`data: {json}\n\n` on the Gemini wire. The owner is asked in the PR to confirm this
in-stream shape as the exhaustion form for Codex and Gemini CLI.

**Post-emission arm added by KBR-247 (owner decisions, 2026-09-14).** KBR-250 closed the
empty ladder's hole and noted an unreachable post-emission arm — its
`test_content_then_empty_finish_never_fires_the_empty_gate` review pins lock the
reachability claim for the content-then-empty-finish ordering, where
`response_was_empty` is recomputed against the accumulated text and the gate does not
fire. **KBR-247 closes the ordering that analysis missed**: an upstream that sends an
**empty finish chunk first** latches `response_was_empty` True at that chunk (`was_empty`
is computed, then `reset()` runs, then the verdict is assigned), and content arriving
after it never recomputes the flag — the gate fires with the client already holding
bytes, and without a guard the empty ladder puts a second attempt's events on that stream.
Each route's empty check now reads a per-request `_request_emitted` flag (declared
outside the attempt loop because `sr` is prepared eagerly on these routes — unlike
`_stream_messages`, where lazy preparation lets `sr is not None` itself answer "has the
request written?") and, when the request has written anything, ends the turn per Q14(a):
buffered finish events dropped, exactly one upstream request, no health charge (the empty
ladder's no-quarantine model). **The terminal shape per route is the route's own existing
in-stream error convention, ratified rather than invented** — the same principle as
(b)'s preamble-hold rationale above:
...
- **/v1/responses**: one SSE `error` event (`responses_format_error`,
  ...
  (`synthesize_completed_events` closes any half-open item the client saw open), then EOF.
  KBR-242's lazy lifecycle (`_write_lifecycle_start`) opens on the first non-finish event,
  so when the guard fires after content the opening is already on the wire; this ending
  reads as a lifecycle that *began* and is closed incomplete.
- **/v1/gemini**: a single SSE `data: {"error": {"code": 502, "message": <message>}}`
...
on Responses (Codex CLI keys the lifecycle off `response.completed`; dropping it would
break the client the route exists to serve). Pre-emission empty verdicts keep today's
ladder on both routes, pinned by `tests/bridge/test_empty_response_retry.py`.
...
historically fall through to the empty-verdict check, and KBR-247 would then have fired
its post-emission guard on top of the exhaustion arm's terminal error event — two
`error` events followed by `response.completed`, violating the "one terminal error"
property above. The fix is a `break` inside the `events_emitted` branch of the
exhaustion arm, matching `_stream_messages` (which always broke after
`finalize_interrupted_stream`) and `_stream_gemini` (which already broke on `events_emitted`
at its exhaustion arm). The no-events branch is untouched: a pre-emission exhausted
attempt still falls through to the empty ladder, which is what AC-2 requires. Regression:
...
pins exactly one `error` event followed by `response.completed` with `status: "incomplete"`
for the empty-then-content-then-error sequence. The Gemini route's parallel arm already
breaks without the fix needing any change, so KBR-247 lands only the Responses branch.
