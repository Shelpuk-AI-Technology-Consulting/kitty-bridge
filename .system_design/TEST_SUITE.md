...
| **No test module imports the `tests` package** | No `tests/**/*.py` module contains `import tests` / `import tests.X` or `from tests import …` at module top-level. Landed with **KBR-280** as `tests/test_no_tests_package_imports.py`. **The trap:** CI's fast-gate invokes bare `pytest` (`.github/workflows/tests.yml`), whose import resolution differs from a developer's `python -m pytest` (which prepends cwd); `tests/` is a rootdir without `__init__.py`, so `import tests.X` resolves under one invocation style and raises `ModuleNotFoundError: No module named 'tests'` under the other — one line taking down the whole six-job matrix. KBR-84 (PR #214) paid a full CI round for exactly this on a contract test's self-guard. The matcher inspects `Module.body` only — the ticket scopes the guard to **direct top-level `import` / `from-import` statements** (the KBR-84 failure shape). Four adjacent shapes are deliberately out of scope: (a) function-level imports, where the same trap lives but widening is a separate, conscious decision; (b) `if TYPE_CHECKING:`-guarded imports, which never execute at runtime under either style and would be a false positive; (c) top-level `try: import tests.X / except ModuleNotFoundError: pass` and other runtime-guarded wrappers (`if`, `with`, `while False:`), where the author has explicitly handled the failure mode — not the accidental KBR-84 shape; (d) dynamic `importlib.import_module("tests.X")`, which an import-statement matcher cannot see (sibling rows state limits of this shape explicitly). Boundaries (a)–(c) are pinned by negative-control tests. The matcher is a shared helper called by both the live scan and the synthetic falsification probes. **No live positive control exists** — the KBR-84 fix removed the only real offender — so the positive control is the synthetic inline case, mirroring `tests/bridge/test_vendor_token_guard.py`'s historical-M13 pattern; the live-scan self-guard asserts the walk covers a non-empty population including the guard file itself. **Why an AST guard, not `pythonpath = ["."]`:** the blanket fix would expose both `harness` (pytest's prepended rootdir) and `tests.harness` as importable — a double-import hazard where one module holds two identities and tests relying on either form can silently mask or break. The guard rejects the pattern without changing import resolution for anything else. **Stated limit:** the guard's rationale assumes `tests/` remains a rootdir without `__init__.py`; adding one would break the bare-import convention (`import layers`, `import internal_key_scan`) that this guard's rationale rests on, and would invalidate the KBR-84 hazard model. |
