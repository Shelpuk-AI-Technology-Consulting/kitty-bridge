...
| P5c | **Raise** `max_tokens` to at least 1025 and set `thinking.budget_tokens = max_tokens - 1`, **discarding the agent's own `budget_tokens`** | same | Thinking enabled | Anthropic requires `budget_tokens >= 1024` and `< max_tokens`. **User-visible**: it increases the agent's own `max_tokens`. **It also costs the prompt cache, and that cost is why this row is a defect rather than a necessity** (KBR-203). Anthropic renders the thinking configuration — `budget_tokens` included — into the prompt, and a change to it always invalidates message-level cache breakpoints, and tool and system ones on some models. The budget shipped here is a function of `max_tokens`, not of what the agent sent, so two requests in one session that differ only in `max_tokens` carry the agent's one budget as two, and cannot share a cached prefix. The row stays *stable* across turns whose `max_tokens` does not move, which is what keeps the cost small today; and on the translated route **M16** already strips almost every breakpoint, so most of the cost is **latent** — it becomes the failure that still charges the 1.25×/2× write premium without the 0.1× read once the KBR-197 epic restores breakpoints. The fix — forward the agent's budget when it is valid — is **KBR-225**; `tests/providers/test_anthropic_thinking_cache_stability.py` pins today's behaviour and must be inverted by it. Native passthrough does **not** meet this row: `tests/bridge/test_native_thinking_passthrough.py` proves the agent's `thinking` reaches the wire byte-for-byte there, and M9's fallback converter carries no thinking at all. The agent's `display` on this branch is P5d's business, not this row's. |
| P5d | Map `_thinking_adaptive` → `thinking: {"type":"adaptive"}`, `_effort` → top-level `effort`, and `_thinking_display` → `thinking.display` on both thinking branches — **or withhold `display`** on adapters whose `forwards_thinking_display` is false (`minimax_token`, `opencode_go`, `zai_coding`) | same | Those keys present | Passthrough of an agent signal, and verbatim is what keeps it cache-safe: Anthropic renders the thinking mode and the effort value into the prompt, so any normalisation of either would cost the prompt cache the way P5c does. **`display` is a mutation only where it is withheld.** Until KBR-203 the translator did not carry it at all, so `{"type":"adaptive","display":"summarized"}` shipped as `{"type":"adaptive"}` on every translated route — and on models whose default is `"omitted"` that hides every thinking token from the user. It is now restored where Anthropic's Messages API defines it (`anthropic`, and `custom_anthropic` when its request is translated) and withheld where the upstream does not document it: MiniMax's Anthropic-compatible reference names `thinking` but not `display`, its translated route exists because MiniMax rejected Claude Code fields before, and `opencode_go`'s Messages route serves MiniMax models. The withholding lands at the same address as the restore, `envelope.extra[thinking]`, which this row and P5c already claim, so no new row is needed; a future ticket that confirms the field on those upstreams flips one class attribute. Top-level `effort` is not Anthropic's documented spelling — `output_config.effort` is, and the translated route drops `output_config` entirely: **KBR-224**. |
