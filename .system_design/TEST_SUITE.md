...
written when the body forces nothing), G34 (`disable_parallel_tool_use: false` omitted) and G35
(a legal `tool_choice` omitted where carrying it would create a failure). **None of the twelve was
found by a guard** — the earlier ones by walking the subscription request path by hand, G28-G30 by
the design and code reviews of KBR-178, G31-G35 by writing and reviewing KBR-214's requirements.
That is the evidence for the sentence above, not a decoration on it. (G36, found the same way, is
a gap in the harness rather than a mutation, so it is not counted here.)
