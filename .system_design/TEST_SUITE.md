...
| M27 | **Carry the upstream CC `tool_calls[].id` onto the emitted Gemini `functionCall.id`** when the upstream carried one; omit when absent | `GeminiTranslator._translate_content` (`kitty/bridge/gemini/translator.py`) | The captured CC reply carries a `tool_calls[].id` | KBR-59 / T-D10 / [KBR-257](https://shelpuk.atlassian.net/browse/KBR-257). The response-direction twin of M18/M19 — same field name, opposite direction. §4 X4 rule: *"emit when present, omit when absent"* (Gemini `FunctionCall.id` is optional per `v1beta`; synthesis stays on the request side, where CC requires an id). Scope is the two Gemini providers (`google_aistudio`, `vertex`). **Not** `NOT_PROJECTABLE` (KBR-195 §8 carries forward): the Gemini reply reader reads `FunctionCall.id` faithfully today (KBR-36, T-A4), so the path is projectable. Anchored at `reply.parts[*].id` (3-segment, the field name) per §3.3.1a — never the bare `reply.parts[*]`, which would also claim a deleted part and is one of §3.3.1's five oracle falsification cases. |
| M28 | Map Ollama `done_reason: "stop"` to `reply.stop_reason = "end_turn"` on the canonical tool-calling exchange | `OllamaCloudAdapter.translate_from_upstream` (`kitty/providers/ollama_cloud.py`) | Captured Ollama reply: `message.tool_calls` non-empty AND `done_reason == "stop"` | KBR-59 / T-D10 / KBR-267 comment 3.1. Ollama's single wire value `"stop"` covers both CC's `"stop"` (text completion) and CC's `"tool_calls"` (tool-calling). `OllamaChatReplyProjection._DONE_REASON_MAP` maps it to `end_turn` per KBR-267's *independent-oracle* rule — inferring `tool_use` from the presence of `message.tool_calls` would read the CC side of the comparison into an Ollama projection. The delta surfaces when the bridge emits on a wire shape using a finer stop-reason vocabulary (CC on the agent side). Anchored at the bare keyed literal `reply.stop_reason`. Scope `("ollama_cloud",)`. |
| M29 | Order parts Text → Thinking → ToolUse → Image (Ollama reader) vs Text → refusal → ToolUse → Thinking (Chat Completions reader) on rare Thinking+ToolUse replies | `OllamaCloudAdapter.translate_from_upstream` (`kitty/providers/ollama_cloud.py`) | Captured Ollama reply: `message.thinking` non-empty AND `message.tool_calls` non-empty | KBR-59 / T-D10 / KBR-267 comment 3.2. The §7.4.1 "two readers disagree about a part boundary report a delta on every subsequent part" rule, applied to the two readers' ordering of Thinking+ToolUse. Anchored at the bare-collection pattern `reply.parts[*]` — the broad anchor that the rule names. The complement (a reply with Text+ToolUse only) produces identical orderings via the empty-content convergence rule and must not fire this row. Scope `("ollama_cloud",)`. **Co-claim note**: M29 (broad, `reply.parts[*]`) and M27 (narrow, `reply.parts[*].id`) co-claim a tool-call-id delta on a Thinking+ToolUse reply under the prefix rule; `_conditional_violations`'s specificity attribution handles this — when M27 is *triggered* on a Thinking+ToolUse reply's id-bearing delta, M29 is exempt (M27 is finer). The corpus builder is told explicitly so the co-claim is not a surprise. |
