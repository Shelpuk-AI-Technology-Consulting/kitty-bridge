...
| a **required field** of a part — a tool `name`, `fileData.fileUri` | **residualise**, project the part: a tool `name` projects `ToolUse(name="")`; `fileData.fileUri` projects with the canonical-JSON digest of the blob as identity | §3.3.1b settles it in those words: "an absent `name` *does* residualise … a call nobody can name cannot be paired or addressed" — the residual makes the non-lossless projection visible. `Image.__post_init__` enforces XOR (KBR-192), so a missing `fileUri` cannot project `Image(ref=None)`; the part keeps its position with the `opaque_digest` recipe as identity. **Stricter raises as deliberate deviations.** The Responses reader follows this row verbatim (`reader_responses.py:1040-1052`, with the residual so the oracle sees it); Anthropic raises on a missing tool_use name (`reader_anthropic_messages.py:750-751`), Chat Completions raises on a non-string name (`reader_chat_completions.py:464-467`), Gemini raises on a non-string name (`reader_gemini.py:623`); the Ollama reader raises on absent / empty / non-string in both directions (KBR-267 reply, KBR-279 request) — `""` is not a lossless projection of a required field (`contract.py:935-941`), and the Ollama request reader additionally never residualised the absent-name case (silent `""` with no residual — the worst of both). The `fileData.fileUri` half keeps its residualise + opaque-digest identity rule (KBR-192). |
