...
word; `tests/test_curl_cffi_transport_contract.py` pins both directions. **`botocore` is untouched**
and still honours the ambient variables — the same hole, on the Bedrock path, unfixed. §6.2.4.
...
| **G10** | Custom-transport containment untested | **Partly closed (KBR-161).** Ambient `NO_PROXY` on `curl_cffi` is now measured, closed with `CURLOPT_NOPROXY`, and pinned; the OAuth refresh leg is covered by the same builder and contract. **Still open:** containment is proven at transport level, never through the bridge; `botocore` still honours the ambient variables; the aiohttp login leg is unproven end to end | §5.5 + §6.2.4 | **2** |
