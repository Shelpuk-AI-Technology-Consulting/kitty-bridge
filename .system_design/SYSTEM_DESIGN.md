...
| T9 | A failed inner kitty holds the pane until Enter | Otherwise the session ends with the pane and every launch error (bad key, egress block, missing binary, Claude refusing the name) disappears. It does not give the outer kitty the inner exit code; that stays tmux's (§3.2). *Product owner.* |
...
- **Live end-to-end** (`tests/integration/test_tmux_disconnect.py`, layer `agent_live`), run by
  ...
  the terminal up (SIGHUP to its process group, `script` killed; `script`'s stdin is a pipe the test
  holds open until then, because `script` forwards end-of-input into the terminal), then proves: the session exists, a
