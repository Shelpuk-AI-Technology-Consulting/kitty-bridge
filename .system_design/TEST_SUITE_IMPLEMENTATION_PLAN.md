...
| KBR-178 | G28, G29 | — | Found while implementing **T-A1** (KBR-33) by reading the Anthropic Messages schema field by field. `MessagesTranslator.translate_request` never read `stop_sequences` or `top_k`, and `server._convert_native_to_cc_format` — a second Messages → CC converter — did not either, so the `tool_use` retry re-dropped both. Product code, so the atomic-PR rule applies: eleven regression tests red at the base revision, evidence in the PR. Fixed at five seams (both converters, plus the three allowlist rebuilds in `anthropic`, `bedrock` and `ollama_cloud`); the fifteen passthrough adapters carry `stop` for free. `top_k` rides the internal `_top_k` because Chat Completions has no field for it — **G28** records what that leaves dropped elsewhere, and **G29** the empty-list omission. No broader guard covers the `stop` half; the internal-key guards (§6.2.3) cover only `_top_k` |
