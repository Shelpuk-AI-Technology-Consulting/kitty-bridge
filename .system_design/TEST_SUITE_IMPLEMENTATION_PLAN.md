...
P1–P25, but the **sub-lettered rows are separate rows with separate triggers**, and the P family
has a gap — P24 is reserved by §9.2's G26 — so the count is **62 published, 61 live**: M13 is
...
off the ranges**; it has moved nine times (M15 with KBR-144, P23 with KBR-171, M16 with KBR-167, M17 with KBR-238, P9d–P9h with KBR-148, P22 with KBR-149, P25 with KBR-185, M18–M25 with KBR-195, P5f with KBR-44)
...
| **T-G4** | ~~Wire-shape honesty guard **at the wire**~~ **DELIVERED — KBR-80 (PR #217), 2026-09-17.** | defect | T-W7, T-B1, T-B2, T-B3 | The declaration agrees with the shape observed at the §3.2.3 boundary, per adapter × model × transport. **The hook-level form already landed with KBR-7** (`tests/test_wire_shape_honesty.py`) and is not this row. What remains is the three `use_custom_transport` adapters, for which nothing at the hook observes the bytes that ship: `openai_subscription` never invokes `translate_to_upstream` on the request path (P13–P17), and `bedrock` and `ollama_cloud` mutate the hook's body in the transport (P18, P19) — those two mutations do not change the shape family, so there the wire check is precautionary. Plus `provider_config`-constructed adapters and native-passthrough requests. **Delivered** by the atomic fix + wire-form guard in `tests/test_wire_shape_honesty_wire.py` (L2): the three custom-transport adapters are captured at the §3.2.3 boundary (boto3/aiohttp/curl_cffi), the sweep extends to `provider_config`-constructed adapters and native-passthrough requests, and the guard caught a live defect on landing (`OpenAISubscriptionAdapter`'s stale `CHAT_COMPLETIONS` declaration, corrected to `RESPONSES`; the four declaration-readers in `server.py` are unreachable for custom transports — analysis in the adapter's `upstream_wire_shape` docstring). See `TEST_SUITE.md` §6.2.3 | §6.2.3 | M |
...
| **T-G7** | SSE grammar state machine | | T-B4, T-W8 | Every stream, including error streams and mid-stream failover, classifies as a complete sentence, error terminal, JSON error, truncation, or malformed per the five-way taxonomy in §6.2.2 (amended by KBR-83: four protocols, not three; the native Messages-wire close-out legitimately leaves blocks open, so "every stream is a complete sentence" was never true) | §6.2.2 | M |
| **T-G8** | aiohttp proxy contract | | T-W5 | Session-level proxy across `>=3.11,<3.14`; a per-request `proxy=None` cannot escape it | §6.2.4 | S |
| **T-G9** | **Header contract** | defect | T-W7 | ~~Exact header set per adapter~~ **DELIVERED — KBR-78, 2026-09-15.** The exact set — names, absences, casing, value shape — is asserted per wire route in `tests/test_upstream_identity_consistency.py` (extending the KBR-8 file, keeping its positive controls), with an AST-derived branch-arm meta-test proving the swept routes execute every `If` arm of every header builder (residual 3). `bedrock` (residual 1) is observed on the botocore-prepared request via a `before-send` short-circuit — no network. The transformed-version residual (residual 2) closes by value-shape literal-equality against `codex_identity`, which the substring sentinel cannot do. The OAuth token POSTs (KBR-161's other half, the C4a scope addition) assert their exact set per site in `tests/test_oauth_leg_identity.py`, and `originator: codex_cli_rs` landed on all four — the ticket's recorded decision, probed live against `auth.openai.com` (2026-09-15, indifferent) | §4.3 C1 | M |
...
| **T-I7** | Streaming recovery — content | | T-B4, T-G7, T-W8 | Four injection points; no duplicated text, no reused tool-call id, no spliced arguments — and, per the answer in §11, the three post-emission points each close the block and terminate with one error rather than recovering. *(Amended by KBR-99: the translated route's transport-class ending is the documented D2 residue — `end_turn` + `message_stop`, pinned, not changed — so "one error" holds for the timeout class; §6.3.1's residue paragraph supersedes this cell for that case. KBR-99 also carries four owner-decided fixes: Gemini's terminal diagnostic, D4 unification, translated streaming D3, and Messages-wire quarantine parity.)* | §6.3.1 | L |
...
| **4** | 10 | T-D1 T-E1 **T-G4 (delivered by KBR-80, PR #217)** T-H2 T-H5 T-I5 T-I7 T-I9 T-I12 T-K5 |
...
| KBR-7 · **CLOSED** | G16 | T-G4 (delivered by KBR-80, PR #217) | Route taken: atomic fix + hook-level guard landed together, red evidence in the PR. **T-G4's wire boundary delivered by KBR-80 (PR #217, 2026-09-17)** — sibling file `tests/test_wire_shape_honesty_wire.py`; the wire form observes the three custom-transport adapters at the §3.2.3 boundary and caught a live defect on landing (openai_subscription's stale CHAT_COMPLETIONS declaration, corrected to RESPONSES). (Status convention: `TEST_SUITE.md` §9.2.) |
| KBR-8 · **CLOSED** | G3 | T-G9 (delivered), TR-1c | Route taken: atomic fix + behavioural guard landed together, red evidence in the PR (2026-09-11). **T-G9 delivered 2026-09-15 (KBR-78)**: exact-set contract per route, AST branch-arm meta-test, bedrock botocore observation, OAuth token POSTs with `originator`, decision recorded. TR-1c still owns the acceptance scenario; G3's policy half waits on Q1. (Status convention: `TEST_SUITE.md` §9.2.) |
| KBR-9 · **CLOSED** | ~~G6~~ | T-G1 | Route taken: guard half landed 2026-09-14 with KBR-76 under the exemption path (T-G1 covers four tables, broader than the single defect); the README correction landed 2026-09-17 in KBR-9 itself — eight bridge-mode routes documented with the `{model:.*}` converter literal. The exempt assertion passed, `UnexpectedExemptionPass` fired, the `t-g1-endpoint-table` row was withdrawn together with its `ratchet` plumbing, and `TestEndpointTable` now gates normally. Status convention: §9.2. |
...
| KBR-149 · **CLOSED** | G23 | — | Route taken: register row **P22** — shape guards + design-doc updates in one atomic change. No product change, so the regression-evidence rule applies to the row itself: the new P22 guard is red at the base revision (row absent) and green with it, evidence in the PR. The gap walk had counted three injection sites; the fourth, `_cc_to_responses`, predates the ticket and injects from the same key, so the row names four. Corpus trigger case + §3.3.2 assertion-2 complement arrive with the T-C entries, as for P3/P4. (Status convention: `TEST_SUITE.md` §9.2.) |
| KBR-185 · **CLOSED** | G27 | — | Route taken: register row **P25** — no product change, owner decision 2026-09-14 to register the drop rather than change the adapter (an empty `include` and an empty `reasoning` carry no instruction, forwarding them to a strictly-validating backend adds a rejection risk for no benefit; the defect was that the drop was undocumented). The new P25 guard is red at the base revision (row absent) and green with it, evidence in the PR. Same shape as KBR-149 (P22): row + design-doc updates + behaviour pin in one atomic change. Two pieces of evidence: the derivation guard (`TestP25ClaimsTheFalsyAllowlistedDrop`) asserts the row exists with the documented shape; the behaviour pins in `TestPrepareResponsesBody` pin the dropped falsy forms, the shipped truthy forms, and the presence-test contrast that `parallel_tool_calls: false` relies on. The §3.2.4 obligation travels with the shape, so P25's Markdown cell carries no backticked pure-word key list — and an explicit assertion pins the absence. `tool_choice` is truthiness-gated on the same chain and is an `envelope.extra` key, but its falsy form is not a legal `CreateResponse` value, so that branch is unreachable today — recorded in P25's Notes and in the row comment, not claimed. P22's interaction (the both-met state: falsy `reasoning` beside a non-`none` effort) is recorded in both the row comment and the Markdown Notes — the rows' claims on `envelope.extra[reasoning]` do not overlap. Corpus trigger case + §3.3.2 assertion-2 complement arrive with the T-C entries, as for P22. (Status convention: `TEST_SUITE.md` §9.2.) |
...
| KBR-183 · **CLOSED** | G26 | T-I7 | Route taken: atomic fix + regression tests, red at base (2 / 6 upstream requests after emission), evidence in the PR (2026-09-13). Widened by the product owner to three sites of the same class found on the way — the Responses and Gemini custom-transport failovers and `openai_subscription`'s in-provider stream-reset retry — plus the pre-emission failover's missing translator reset. Found while answering **Q14** (KBR-163), by checking the claim that the answer merely ratified existing behaviour. It does not: `_is_transport_error` excludes `asyncio.TimeoutError` by design and the failover arm carries no emission test, so a post-emission timeout still writes a second attempt onto the client's open stream. Measured, not inferred. Product code, so the atomic-PR rule applies — the regression test must be red at the base revision |
...
| KBR-214 | G31, G32, G33, G34, G35, G36 | Internal-key guards (§6.2.3) now cover `_metadata`; nothing broader covers the `tool_choice` half | Found while closing **KBR-178** (PR #92), in the same fixture and at the same seam. `MessagesTranslator.translate_request` and `server._convert_native_to_cc_format` never read `tool_choice` or `metadata`, so a forced tool call became optional and a forbidden one possible. Product code, so the atomic-PR rule applies: 38 regression tests red at the base revision, evidence in the PR. Fixed through **one shared helper** called by both converters (a spy test holds the call, since parity cannot see a faithful copy), plus three rebuild seams — `anthropic` (and its four delegates), `bedrock`, and `openai_subscription._cc_to_responses`, whose verbatim copy would otherwise have shipped Chat Completions' nested named form to the Responses backend. The PR review then found that same builder dropping `parallel_tool_calls`, which the Messages ingress now produces and the Codex allowlist accepts; it is carried, and G26's count falls from seventeen to sixteen. The sixteen passthrough adapters carry it for free. `metadata` rides `_metadata` to the Anthropic family only. **Side effect:** those seams are shared with the Chat Completions ingress, so a CC client's `tool_choice` now reaches the same three destinations too. **G31–G35** record what is deliberately left dropped, rewritten or omitted — including the WebSearch sub-call's forced `web_search` server tool, omitted on the product owner's decision — and **G36** the harness's missing address for the parallel knob. Sibling defect on the Responses and Gemini ingresses filed as **KBR-221** |
| KBR-226 · **CLOSED** | G39 | — | Found while implementing **KBR-183**, by its design review, then confirmed by running the translator. On every translated `/v1/messages` route the tool-call branch opened every `tool_use` at the same Anthropic block index and never advanced the counter, so a stream with two parallel tool calls was not a sentence in the §6.2.2 grammar (both calls at index 0, doubled stops at index 0). Product code, so the atomic-PR rule applies: 5 L1 translator tests + 1 server-level stream walk, red at the base revision (watched failing with the fix line removed), evidence in the PR. Fixed by advancing the counter when a tool block opens — the finish, `finalize_interrupted_stream` and `close_open_blocks` paths close by the recorded block index and needed no change. G39 records the decided stream shape (overlap permitted between blocks with distinct indices; stop order free; per-index start-once/stop-once and no delta outside its window), the scope-out (a repeated id-chunk for an open Chat Completions index stays malformed), and why advancing cannot flip the empty-stream fallbacks. The ticket's sibling check (DoD item 3): Gemini clean; the Responses translator has the same class through a different mechanism — text and the first function call both at `output_index: 0` — probed, duplicate-swept, and filed as **KBR-240** rather than folded into this fix, after the KBR-221 precedent. (Status convention: `TEST_SUITE.md` §9.2.) |
| KBR-240 · **CLOSED** | G40 | — | Found by **KBR-226**'s sibling check (its DoD item 3), confirmed by running the translator. On every translated `/v1/responses` route `ResponsesTranslator` had no output-item counter at all: reasoning and text pinned to `output_index: 0`, and a function call took the CC `tool_calls[].index` raw — also 0 for the first call — so prose-then-tool-call (the common Codex shape) announced two items at slot 0 and closed slot 0 twice; reasoning vs text collided the same way. Product code, so the atomic-PR rule applies: 10 L1 translator tests + 1 server-level stream walk, red at the base revision (watched failing with the fix removed), evidence in the PR. Fixed by one shared counter allocated when an item opens — chunk path, finish path and `synthesize_completed_events` close by the recorded index; the CC index remains a routing key only. The ticket's close-path audit then widened the fix by two found holes: the EOF path now closes the reasoning item (it never did — an added with no done on every timeout/dropped-connection stream), and every opened item appears in `response.completed`'s `output`, so array position equals `output_index` (a text item stripped to nothing by thinking-tag removal closes empty and stays in the array). The arguments delta/done events gained the vendor-required `output_index` (DoD item 3). G40 records the decided stream shape (G39's), the scope-outs (non-streaming `translate_response` needs no allocation; a repeated id-chunk stays malformed) and why allocation cannot flip the empty-stream fallbacks. The design review's mutation pass caught the first suite under-pinning the text part/delta/done indices at non-zero slots; the mixed-stream test now kills that mutant. (Status convention: `TEST_SUITE.md` §9.2.) |
...
| §4.3 C1 | Header contract | T-G9 — **delivered (KBR-78, 2026-09-15)** |
...
| §6.2.3 | Register, internal-key, wire-shape, vendor-token, docs guards | T-G1–T-G5 — T-G3 delivered by KBR-6, the **hook-level** half of wire-shape honesty by KBR-7, and the **wire-level** half by KBR-80 (PR #217, 2026-09-17; sibling file `tests/test_wire_shape_honesty_wire.py`) |
