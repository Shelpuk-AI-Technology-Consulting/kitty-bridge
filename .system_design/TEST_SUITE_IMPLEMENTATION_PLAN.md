...
| KBR-9 | G6 | T-G1 | Route taken: guard half landed 2026-09-14 with KBR-76 under the exemption path (T-G1 covers four tables, broader than the single defect); the README correction remains in KBR-9. Evidence in PR: the endpoint agreement assertion fails at the base revision with exactly KBR-9's four diffs (README names `POST /v1/gemini/generateContent`, omits `/v1beta/models/{model:.*}:generateContent`, `:streamGenerateContent`, `GET /v1/models`). Status convention: §9.2. |
